...
void drawGame() {
  ...
  else if (!dead) runGame();
  if (dead == true && initialized) gameOver();
...
void gameOver() {
  GameGraph.background(0);
  GameGraph.textSize(40);
  GameGraph.text("GameOver", width/3, height/2);
}
...
void deathCheck() {
  if (Player.playerBody.size() < 2 || Player.playerPoints.size() < 2) return;
  //hitbox
  Points hitbox = Player.playerPoints.get(Player.playerPoints.size()-1);
  for (int i = 1; i <= Player.playerBody.size() - 1; i++) //skip the head
  {
    PlayerBody currentBody = Player.playerBody.get(i);
    float dx = hitbox.xPos - currentBody.xPos;
    float dy = hitbox.yPos - currentBody.yPos;
    float distanceSq = dx * dx + dy * dy;

    if (distanceSq <= (playerBallSize/2) * (playerBallSize/2)
    || hitbox.xPos > width || hitbox.xPos < 0 
    || hitbox.yPos > height || hitbox.yPos < 0)
    {
      dead = true;
      println(Player.playerBody.size());
      return;
    }
  }
}
...
class PlayerSnake {
  ...
  void move() {
    ...
    //DEBUG draw points on playerpoints (should not be visible and inside snake)
    ...
    for (int i = 0; i <= tailSize; i++) {
      ...
      tempBody.add(new PlayerBody(p.xPos, p.yPos, pointIndex)); //xPos and yPos adjusted using pointIndex
